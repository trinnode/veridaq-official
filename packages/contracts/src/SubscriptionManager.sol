...
contract SubscriptionManager is AccessControl {
    ...
    enum InstitutionTier { FREE, PAID }
    ...
    function setInstitutionTier(bytes32 institutionId, InstitutionTier tier)
    ...
    function shouldSponsor(bytes32 institutionId, uint256 batchSize) external view returns (bool) {
        return institutionTiers[institutionId] == InstitutionTier.FREE
            && batchSize <= FREE_TIER_BATCH_LIMIT;
    }

    // ── Employer verification counter ─────────────────────────────────────────

    /// @notice Called by the platform admin when a new employer passes KYC.
